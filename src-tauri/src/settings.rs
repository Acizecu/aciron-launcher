    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            if is_writable(dir) {
                ...
                // Основной канал остаётся ПРЯМО рядом с exe — так было всегда, и
                // менять это нельзя. У установленных лаунчеров данные лежат
                // именно там; подпапка означала бы для каждого обновившегося
                // сброшенные пути к папкам, потерянные аккаунты и пустой список
                // сборок. Механизм подхвата старых данных это бы вытянул, но
                // ценой диалога переноса при обычном обновлении.
                #[cfg(not(feature = "dev-channel"))]
                {
