...
const MOJANG_OFFLINE: &str = "Minecraft is not responding right now, try again later";
...
async fn mc_token(a: &Account) -> Result<String, String> {
    if !(a.kind == "microsoft" || (a.kind == "aciron" && a.licensed)) {
        return Err("No licence is linked".into());
    }
    fresh_session(&a.id)
        .await
        .map(|x| x.access_token)
        .map_err(|_| "your Microsoft sign-in has expired — relink the license".to_string())
}

pub(crate) async fn fresh_session(id: &str) -> Result<Account, String> {
    let _guard = SESSION_LOCK.lock().await;
    let a = accounts::get_account(id).ok_or("Account not found")?;
    ...
    accounts::get_account(id).ok_or_else(|| "Account not found".to_string())
}
...
async fn ok(r: reqwest::Response) -> Result<(), String> {
    ...
    Err(match code {
        401 | 403 => "your Microsoft sign-in has expired — relink the license".into(),
        429 => "Mojang asks you to wait, the look was changed too often".into(),
        _ => {
            let snippet: String = body.chars().take(160).collect();
            format!("Minecraft returned an error: {code} {snippet}")
        }
    })
}
...
fn require_64(png: &[u8]) -> Result<(), String> {
    ...
    if png.len() < 24 {
        return Err("The skin file is damaged".into());
    }
    ...
    Err(format!(
        "Minecraft only accepts 64×64 skins: {w}×{h}"
    ))
}
...
async fn try_sync() -> Result<bool, String> {
    ...
    let a = accounts::get_account(&snapshot.id).ok_or("Account not found")?;
...
async fn sync_skin(
    ...
    if has_skin {
        ...
        let png = http()?
            ...
            .map_err(|_| OFFLINE.to_string())?
            .error_for_status()
            .map_err(|_| "Couldn't fetch your skin from Aciron".to_string())?
            .bytes()
            .await
...
pub async fn set_cape(cape_id: Option<String>) -> Result<(), String> {
    let _guard = SYNC_LOCK.lock().await;
    let a = accounts::active_account().ok_or("No active account")?;
    let token = mc_token(&a).await?;
    ...
    if cape_id.is_some() && !a.aciron_token.is_empty() {
        ...
        let r = crate::aciron::post("/api/wardrobe/cape/license")?
            ...
            .map_err(|_| OFFLINE.to_string())?;
        if !r.status().is_success() {
            return Err("the license cape is on, but your own cape could not be taken off — remove it in the wardrobe".into());
        }
    }
    Ok(())
}
...
pub async fn prove_license(aciron_token: &str, ms: &Account) -> Result<(), String> {
    let resp = crate::aciron::post("/api/license/challenge")?
        ...
        .map_err(|_| OFFLINE.to_string())?;
    if !resp.status().is_success() {
        return Err("Aciron ID did not issue a licence check token".into());
    }
    let ch: Value = resp.json().await.map_err(|e| e.to_string())?;
    let server_id = ch["serverId"]
        .as_str()
        .ok_or("Aciron ID did not issue a licence check token")?
        .to_string();
    ...
    if !verify.status().is_success() {
        ...
        return Err(serde_json::from_str::<Value>(&body)
            ...
            .unwrap_or_else(|| "Aciron ID did not confirm the licence".into()));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn reads_active_profile_textures() {
        let p = json!({
            "skins": [
