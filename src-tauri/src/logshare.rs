...
pub async fn log_share(game_id: String) -> Result<SharedLog, String> {
    if LOG_URL.is_empty() {
        return Err("Публикация логов в этой сборке не настроена".into());
    }
    let path = log_path(&game_id);
    let raw = std::fs::read(&path).map_err(|_| {
        "Лог не найден. Он появляется после запуска игры и лежит в папке logs.".to_string()
    })?;
    if raw.is_empty() {
        return Err("Лог пуст — публиковать нечего".into());
    }
    ...
    let cl = http().ok_or("Не удалось подготовить соединение")?;
    let resp = cl
        ...
        .send()
        .await
        .map_err(|e| format!("Не удалось отправить лог: {e}"))?;

    if !resp.status().is_success() {
        return Err(format!("Сервис логов ответил {}", resp.status()));
    }
    let v: serde_json::Value = resp
        .json()
        .await
        .map_err(|e| format!("Непонятный ответ сервиса: {e}"))?;

    let id = v["id"].as_str().unwrap_or_default().to_string();
    if id.is_empty() {
        return Err("Сервис не вернул ссылку".into());
    }
    Ok(SharedLog {
        url: v["url"]
            .as_str()
...
mod tests {
    ...
    #[test]
    fn токен_вырезается_вместе_со_значением() {
        let got = mask_after("args [--accessToken, eyJhbGci.QUJD.sig, --uuid]", &["--accessToken"]);
        assert!(!got.contains("eyJhbGci"), "токен остался: {got}");
        assert!(got.contains("--accessToken"), "имя ключа потерялось: {got}");
        assert!(got.contains("--uuid"), "хвост строки потерялся: {got}");
    }

    #[test]
    fn регистр_ключа_не_важен() {
        let got = mask_after("SESSIONID=abcdef123456", &["sessionId"]);
        assert!(!got.contains("abcdef"), "{got}");
    }

    #[test]
    fn ключ_без_значения_не_ломает_строку() {
    ...
    fn несколько_вхождений_подряд() {
        ...
        assert_eq!(got.matches("<вырезано>").count(), 2, "{got}");
    }

    #[test]
    fn кириллица_в_строке_не_режется() {
    ...
    #[test]
    fn середина_обрезается_с_отметкой() {
        let big = "a".repeat(HEAD_KEEP + TAIL_KEEP + 100_000);
        let got = clamp(&big);
        assert!(got.len() < big.len());
        assert!(got.contains("пропущено"), "нет отметки об обрезке");
    }

    #[test]
    fn короткий_лог_не_трогаем() {
        let s = "две\nстроки";
        assert_eq!(clamp(s), s);
    }

    #[test]
    fn обрезка_не_рвёт_символ() {
