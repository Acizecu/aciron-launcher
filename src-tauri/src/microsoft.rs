...
fn open_auth_window(app: &AppHandle, url: &str) -> Option<tauri::WebviewWindow> {
    ...
    match tauri::WebviewWindowBuilder::new(app, AUTH_WINDOW, tauri::WebviewUrl::External(parsed))
        .title(crate::i18n::t("Sign in with Microsoft"))
        ...
        .build()
    {
        Ok(w) => Some(w),
        Err(e) => {
            eprintln!("[microsoft] sign-in window did not open ({e}), falling back to the browser");
            None
        }
    }
}
...
pub async fn interactive_login(app: AppHandle) -> Result<Account, String> {
    if client_id().is_empty() {
        return Err("Microsoft sign-in is not configured in this build (no client_id)".into());
    }
    ...
    let code = tokio::select! {
        ...
        _ = closed_rx => Err("The Microsoft sign-in window was closed — sign-in cancelled".to_string()),
    };
    if let Some(w) = &auth_window {
        let _ = w.close();
    }
    ...
    if let Some(err) = tok.get("error").and_then(|v| v.as_str()) {
        ...
        return Err(format!("Microsoft sign-in failed: {desc}"));
    }
    let ms_access = tok["access_token"]
        .as_str()
        .ok_or("Microsoft: no access token received")?
        .to_string();
...
async fn wait_for_code(
    ...
    let accept = async {
        loop {
            ...
            if let Some(err) = params.get("error") {
                reply(
                    &mut stream,
                    crate::i18n::t("Sign-in failed. You can close this tab and go back to the launcher."),
                )
                .await;
                return Err(format!("Microsoft sign-in failed: {}", urldecode(err)));
            }
            if let Some(code) = params.get("code") {

                let got_state = params.get("state").map(|s| urldecode(s)).unwrap_or_default();
                if got_state != expected_state {
                    eprintln!("[microsoft] OAuth: state mismatch, request rejected");
                ...
                reply(
                    &mut stream,
                    crate::i18n::t("All set! Head back to Aciron Launcher — this tab can be closed."),
                )
                .await;
    ...
    match tokio::time::timeout(timeout, accept).await {
        Ok(r) => r,
        Err(_) => Err("Sign-in timed out, please try again".into()),
    }
}
...
pub async fn refresh_account(refresh_token: &str) -> Result<Account, String> {
    if refresh_token.is_empty() {
        return Err("No refresh token — sign in to Microsoft again".into());
    }
    let cl = http()?;
    let tok: Value = cl
        .post(TOKEN_URL)
    ...
    let ms_access = tok["access_token"]
        .as_str()
        .ok_or("The Microsoft session has expired — sign in again")?
        .to_string();
...
async fn finish_login(
    ...
    let xbl_token = xbl["Token"]
        .as_str()
        .ok_or("Xbox Live: could not get a token")?
        .to_string();
    ...
    if let Some(xerr) = xsts.get("XErr").and_then(|v| v.as_u64()) {
        let msg = match xerr {
            ...
            2148916236 | 2148916237 => "Xbox requires age verification",
            2148916238 => "This is a child account — add it to a Microsoft family group",
            _ => "Could not sign in to Xbox (XSTS)",
        };
        return Err(msg.into());
    }
    let xsts_token = xsts["Token"]
        .as_str()
        .ok_or("XSTS: could not get a token")?
        .to_string();
    let uhs = xsts["DisplayClaims"]["xui"][0]["uhs"]
        .as_str()
        .or_else(|| xbl["DisplayClaims"]["xui"][0]["uhs"].as_str())
        .ok_or("XSTS: uhs is missing")?
        .to_string();
    ...
    let mc_token = mc["access_token"]
        ...
        .ok_or_else(|| {
            ...
            format!("Could not sign in to Minecraft: [{mc_status}] {snippet}")
        })?
        .to_string();
