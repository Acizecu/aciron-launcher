...
fn walk(
    ...
    let rd = std::fs::read_dir(dir)
        .map_err(|e| format!("Cannot read the folder: {} — {e}", dir.display()))?;
    for e in rd {
        let e = e.map_err(|err| format!("Cannot read the folder: {} — {err}", dir.display()))?;
        let path = e.path();
...
pub async fn export_build(
    ...
    emit_op(&app, "export", "modpack", "Reading instance files", 0, 1);
    ...
    if files.is_empty() {
        return Err("Nothing to export: nothing is selected".into());
    }
    ...
    let out = std::fs::File::create(&tmp_path).map_err(|e| format!("Cannot create the file: {e}"))?;
    ...
    let mut pack = || -> Result<(), String> {
        for (rel, path) in &files {
            ...
            if done % 8 == 0 || done == total {
                emit_op(&app, "export", "modpack", "Packing", done, total);
            }
            ...
            let bytes =
                std::fs::read(path).map_err(|e| format!("Cannot read the file: {rel} — {e}"))?;
    ...
    let manifest = if format == "mrpack" {
        ...
        (
            ...
            json!({
                ...
                "versionId": "1.0.0",
                "name": build.name,
                "summary": crate::i18n::t("Exported from Aciron Launcher"),
    ...
    std::fs::rename(&tmp_path, &dest_path).map_err(|e| {
        let _ = std::fs::remove_file(&tmp_path);
        format!("Cannot save the archive: {e}")
    })?;

    emit_op(&app, "export", "done", "Instance exported", 1, 1);
...
async fn download_file(cl: &reqwest::Client, url: &str, path: &Path) -> Result<(), String> {
    ...
    if !url.starts_with("https://") {
        return Err("The link is not https".into());
    }
...
async fn resolve_remote(
    ...
    emit_op(
        ...
        "export",
        "modpack",
        "Looking files up on CurseForge",
        0,
        pending.len() as u64,
    );
    ...
    for (rel, m, local_sha1) in pending {
        ...
        emit_op(app, "export", "modpack", "Looking files up on CurseForge", done, total);
...
pub async fn import_acpack(app: AppHandle, path: String) -> Result<ImportResult, String> {
    ...
    emit_op(&app, "install", "done", "Instance imported", 1, 1);
    Ok(ImportResult { build, missing })
}

fn unpack_acpack(app: &AppHandle, path: &str) -> Result<(Build, Vec<RemoteFile>), String> {
    emit_op(app, "install", "modpack", "Reading instance file", 0, 1);

    let file = std::fs::File::open(path).map_err(|e| format!("Cannot open the file: {e}"))?;
    let mut zip = zip::ZipArchive::new(file).map_err(|_| "This is not an .acpack archive")?;

    let manifest: Value = {
        let mut f = zip
            .by_name("aciron.pack.json")
            .map_err(|_| "The archive has no aciron.pack.json — it is not an Aciron instance")?;
        let mut s = String::new();
        f.read_to_string(&mut s).map_err(|e| e.to_string())?;
        serde_json::from_str(&s).map_err(|e| format!("Corrupted manifest: {e}"))?
    };

    let name = manifest["name"].as_str().unwrap_or_else(|| {
        crate::i18n::t("Imported instance")
    });
    let mc = manifest["mc_version"]
        .as_str()
        .filter(|s| !s.is_empty())
        .ok_or("The archive does not state a Minecraft version")?;
    ...
    for i in 0..zip.len() {
        if i % 16 == 0 {
            emit_op(app, "install", "modpack", "Unpacking", i as u64, total);
        }
        ...
        if let Some(p) = dest.parent() {
            std::fs::create_dir_all(p).map_err(|e| format!("Cannot create the file: {rel} — {e}"))?;
        }

        let mut out =
            std::fs::File::create(&dest).map_err(|e| format!("Cannot create the file: {rel} — {e}"))?;
        std::io::copy(&mut entry, &mut out)
            .map_err(|e| format!("Cannot unpack the file: {rel} — {e}"))?;
    }
...
async fn fetch_remote(app: &AppHandle, build: &mut Build, remote: &[RemoteFile]) -> Vec<String> {
    ...
    emit_op(app, "install", "modpack", "Downloading mods", 0, total);
    ...
    let tasks = owned.into_iter().map(|(i, r)| {
        ...
        async move {
            ...
            emit_op(&app, "install", "modpack", "Downloading mods", n, total);
            (i, outcome)
        }
    });
