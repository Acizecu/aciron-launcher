...
export default function CreateBuildModal({
  ...
  useEffect(() => {
    let alive = true;
    setSupported(null);
    void loaderGameVersions(loader).then((list) => {
      if (alive) setSupported(list.length > 0 ? new Set(list) : null);
    });
    return () => {
      alive = false;
    };
  ...
  useEffect(() => {
    if (loader !== "forge") setWithOptifine(false);
  }, [loader]);
  ...
  const versionOptions = useMemo(
    () =>
      (versions ?? [])
        ...
        .filter((v) => !supported || supported.has(v.id)),
    [versions, showSnapshots, supported]
  );

  useEffect(() => {
    if (!version || versionOptions.length === 0) return;
    if (!versionOptions.some((v) => v.id === version)) setVersion(versionOptions[0].id);
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (withOptifine) {
        try {
          await installOptifine(build.id);
        } catch (e) {
          onCreated();
  ...
  const pickImage = async () => {
    ...
    setImagePreview(await readImageDataUrl(f).catch(() => ""));
  };
