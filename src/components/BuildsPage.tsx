...
const BUILDS_PER_PAGE_FALLBACK = 9;
...
export default function BuildsPage() {
  ...
  const [perPage, setPerPage] = useState(BUILDS_PER_PAGE_FALLBACK);
  const gridMounted = builds.length > 0;
  useEffect(() => {
    const grid = gridRef.current;
    if (!grid) return;
    const measure = () => {
      const cols = getComputedStyle(grid).gridTemplateColumns.split(" ").filter(Boolean).length;
      if (cols > 0) setPerPage(cols * BUILDS_ROWS);
    };
    measure();
      ...
      <div className="min-h-0 flex-1 overflow-y-auto pr-1">
        {builds.length === 0 ? (
          ...
          {builds.length > 0 && (() => {
            ...
            const pageBuilds = ordered.slice(safePage * perPage, safePage * perPage + perPage);
            return (
              <>
                {}
                <div
