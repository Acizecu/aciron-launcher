            ...
            {cat === "java" && (
              <>
                ...
                <Card>
                  ...
                  <Field label={t("JVM-аргументы")} hint={t("Флаги виртуальной машины при запуске, через пробел")} column>
                    <input
              ...
              <>
                ...
                <Card>
                  ...
                  <Field
                    ...
                    hint={t("Игра откроется на весь экран")}
                  >
                    <Toggle value={s.fullscreen} onChange={(v) => update({ fullscreen: v })} />
                  </Field>
                  <Field label={t("Размер окна игры")} hint={t("Когда полноэкранный режим выключен")}>
            ...
            {cat === "behavior" && (
              <>
                ...
                <Card>
                  ...
                  <Field label={t("Звук наведения")} hint={t("Короткий звук при наведении на кнопку")}>
                    <Toggle
                      value={sfx.hover}
                      onChange={(v) => {
                        setSfx((p) => ({ ...p, hover: v }));
                ...
                <Card>
                  ...
                  <Field
                    ...
                    hint={t("Лаунчер запускается значком в трее, без окна. Чтобы открыть его, щёлкните по значку.")}
                  >
                    <Toggle
                      value={autostart}
                      onChange={(v) => {
                  ...
                  <Field
                    label={t("Серверы Aciron в списке")}
                    hint={t("Добавляет пять популярных серверов в список")}
                  >
                    <Toggle value={s.autoadd_server} onChange={(v) => update({ autoadd_server: v })} />
                  </Field>
                  <Field
                    label={t("Звук уведомлений")}
                    hint={t("Звук при заявке в друзья и других уведомлениях")}
                  >
                ...
                <Card>
                  ...
                  <Field
                    label={t("Не обновляться, пока идёт работа")}
                    hint={t("Отключает и проверку, и установку. Пригодится, пока правите код.")}
                  >
                    <Toggle
        ...
        <Modal
          ...
          subtitle={
            crashPending > 0
              ...
              : t("Отчётов нет. Так выглядел бы отчёт о сбое, случись он сейчас")
          }
