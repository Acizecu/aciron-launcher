  ...
  return (
    <>
      <button
        ref={btn.ref}
        onClick={btn.toggle}
        className={`flex h-10 w-full items-center gap-2.5 rounded-xl border px-3 text-sm text-text transition-colors ${
          btn.open ? "border-accent/60 bg-card" : "border-border bg-card hover:border-accent/40"
        }`}
      >
        <span className="grid h-6 w-6 shrink-0 place-items-center rounded-[6px] bg-bg text-accent">
          <i className={`${cur.icon} text-[11px]`} />
        </span>
      ...
      <AnchoredPanel
        ...
        onClose={btn.close}
        height={330}
        className="rounded-2xl border border-border bg-panel p-2.5 shadow-xl"
      >
            ...
            return (
              <button
                ...
                className={`dropdown-item-in group flex w-full flex-col gap-2 rounded-[14px] border-1 p-2.5 text-left transition-colors ${
                  on ? "border-accent bg-card" : "border-[#232427]/65 hover:border-accent/40"
                }`}
              >
                {}
                <div className="relative h-14 w-full overflow-hidden rounded-[10px] border border-border bg-bg">
                  <i
                    className={`${b.icon} pointer-events-none absolute -left-1 top-1/2 -translate-y-1/2 text-[56px] leading-none opacity-[0.14]`}
                  />
                </div>
                <div className="flex items-center justify-between">
                  <span className={`truncate text-xs font-medium ${on ? "text-accent" : "text-text"}`}>
                    {bgLabel(b.id as BackgroundId)}
                  </span>
                  {on && <i className="fa-solid fa-circle-check text-xs text-accent" />}
                </div>
              </button>
            );
          })}
